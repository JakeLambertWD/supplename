	...
	return (
		<>
			<Flex
				...
				px={{ base: 'sm', md: 50 }}
				py='lg'
				bg={pathname.startsWith('/works/') ? theme?.colors?.primary?.[9] : scrolled ? theme?.colors?.primary?.[9] : 'transparent'}
				style={{ zIndex: 45 }}
			>
				<Image
				...
				<Dropdown hiddenFromSm open={open} />
					...
					{navigationLinks.map(link => {
						...
						return (
							<NavLink
								...
								style={{
									borderBottom: isActive ? `2px solid ${theme?.colors?.primary?.[1]} !important` : 'none'
								}}
							/>
						);
					})}
